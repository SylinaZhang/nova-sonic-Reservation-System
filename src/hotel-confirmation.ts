...
const generateReservationId = (): string => {
    return `RES-${Math.floor(Math.random() * 100000).toString().padStart(5, '0')}`;
};

// Helper to create a default cancellation policy
const createDefaultCancellationPolicy = (reservationId: string, checkInDate: string): CancellationPolicy => {
    // Calculate dates for cancellation policy (7 days and 2 days before check-in)
    const checkIn = new Date(checkInDate);
    
    const freeCancellationDate = new Date(checkIn);
    freeCancellationDate.setDate(checkIn.getDate() - 7);
    
    const partialRefundDate = new Date(checkIn);
    partialRefundDate.setDate(checkIn.getDate() - 2);
    
    return {
        reservationId: reservationId,
        freeCancellationUntil: freeCancellationDate.toISOString().split('T')[0],
        partialRefundUntil: partialRefundDate.toISOString().split('T')[0],
        partialRefundPercentage: 50,
        noRefundAfter: partialRefundDate.toISOString().split('T')[0],
        additionalNotes: null
    };
};
...
export const createReservation = async (params: CreateReservationParams): Promise<CreateReservationResult> => {
    console.log(`Creating new reservation for ${params.name} with check-in date ${params.checkInDate}`);
    
    // Check if a reservation with the same name and check-in date already exists
    ...
    if (existingReservation) {
        return {
            ...
            reservation: null,
            confirmationCode: "",
            message: "A reservation with this name and check-in date already exists"
        };
    }
    ...
    // Add the reservation to the mock database
    mockReservations.push(newReservation);
    
    // Create a cancellation policy for the new reservation
    const cancellationPolicy = createDefaultCancellationPolicy(reservationId, params.checkInDate);
    mockCancellationPolicies[reservationId] = cancellationPolicy;
    
    // Generate confirmation code
    const confirmationCode = `CONF-${Math.floor(Math.random() * 1000000).toString().padStart(6, '0')}`;
...
export const processToolCalls = async (toolName: string, toolInput: any): Promise<any> => {
    switch (toolName) {
        ...
        case 'createReservationTool':
            return await createReservation(toolInput);
